...
function rateRestaurants(db, query, r,callback) {
  db.collection('restaurants').update(query, {$push: r},function(err,result) {
    assert.equal(err,null);
...
function checkRestaurant(db,query1,callback){ 
  var result = [];
  var cursor = db.collection('restaurants').find(query1);
...
function checkRateRestaurant(db,query1,query2,callback){ 
  var result = [];
  var cursor = db.collection('restaurants').find({query1,query2});
